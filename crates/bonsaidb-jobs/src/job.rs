...
impl<Q: Queueable> Job<Q> {
    pub async fn update<Database: Connection>(
        &mut self,
        database: &Database,
    ) -> Result<bool, bonsaidb_core::Error> {
        if let Some(doc) = schema::Job::get(self.0.header.id, database).await? {
            self.0 = doc;
            Ok(true)
        } else {
            Ok(false)
    ...
    pub async fn wait_for_result<Database: Connection + PubSub>(
        &mut self,
        database: &Database,
    ) -> JobResult<Q> {
        loop {
            let subscriber = database.create_subscriber().await?;
            subscriber.subscribe_to(job_topic(self.0.header.id)).await?;
            ...
            return if let Some(result) = &self.0.contents.result {
                ...
                match subscriber.receiver().recv_async().await {
                    Ok(message) => {
                        <Q::Format as OwnedDeserializer<JobResult<Q>>>::deserialize_owned(
                            &Q::format(),
                            &message.payload,
...
pub trait Executor {
    ...
    async fn execute_with_progress<Database: Connection + PubSub>(
        ...
        let result = loop {
            tokio::select! {
                ...
                progress = job_handle.receiver.changed() => {
                    ...
                    job.contents.progress = job_handle.receiver.borrow_and_update().clone();

                    match job.update(database).await {
                        Ok(()) => {}
                        Err(bonsaidb_core::Error::DocumentConflict(..)) => {
                            if let Some(updated_job) = schema::Job::get(job.header.id, database).await? {
                                *job = updated_job;
                                job_handle.cancel.send(job.contents.cancelled_at).unwrap();
                            } else {
                                break Ok(None)
                            }
                        }
                        Err(other) => break Err(<Self::Job as Queueable>::Error::from(other))
                    }
                }
            }
        };
        ...
        job.contents.result = Some(result_bytes.clone());
        job.contents.returned_at = Some(Timestamp::now());
        job.update(database).await?;

        database
            .publish_raw(job_topic(job.header.id), result_bytes.0)
            .await?;
